...
/**
 * Universal Place Card Component for Prayagraj.
 * Whole-card clickability to /destinations/prayagraj/:slug
 *
 * @param {Object} props
...
export function PlaceCard({ place, className = '' }) {
  ...
  const targetUrl = `/destinations/prayagraj/${place.slug || place.id}`;
    ...
    >
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        ...
        <div className="space-y-2 text-xs text-text-mid border-t border-white/10 pt-3">
          ...
          {(place.ticket || place.ticketPrice) && (
            <div className="flex items-center justify-between text-[11px] font-mono text-text-low">
              <span className="flex items-center gap-1">
                ...
                <span>{place.ticket?.indian || place.ticketPrice || 'Free Entry'}</span>
              </span>
            </div>
          )}
        </div>
