...
/**
 * Delhi City Destination Page Component.
 * Route: /destination/north/delhi or /destinations/delhi
 */
export function DelhiPage() {
  ...
  useEffect(() => {
    ...
    carouselTimerRef.current = setInterval(() => {
      setCurrentHeroIndex((prev) => (prev + 1) % DELHI_HERO_CAROUSEL.length);
    }, 4500);
  ...
  // Filtered places
  const filteredPlaces = DELHI_PLACES.filter(
    (place) => place.category === activeCategory
  );

  const activeHeroItem = DELHI_HERO_CAROUSEL[currentHeroIndex];

  return (
    <div className="min-h-screen bg-bg-base text-text-high pb-16">
      {/* 1. HERO SECTION: AUTO-SLIDING PHOTO CAROUSEL */}
      <section
        className="relative w-full h-[65vh] min-h-[480px] max-h-[720px] overflow-hidden group select-none"
        onMouseEnter={() => setIsCarouselPaused(true)}
        onMouseLeave={() => setIsCarouselPaused(false)}
      >
        {/* Background Image Carousel Slides */}
        {DELHI_HERO_CAROUSEL.map((slide, index) => (
          <div
            key={slide.id}
            className={cn(
        ...
        <div className="absolute inset-0 z-20 flex flex-col justify-end max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 sm:pb-16 pointer-events-none">
          <div className="max-w-3xl space-y-3">
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full glass-chip text-amber-300 border border-amber-500/30 text-xs font-semibold uppercase tracking-wider font-mono">
              ...
              <span>Capital Territory of Delhi</span>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2 text-xs text-text-mid font-medium pointer-events-auto">
              <span className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-black/40 backdrop-blur border border-white/10">
              ...
              <span className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-black/40 backdrop-blur border border-white/10">
                ...
                <span>Live Crowd: Moderate</span>
              </span>

              <Link
                to={`/destination/north/delhi/${activeHeroItem.id}`}
        ...
        <button
          ...
          onClick={() =>
            setCurrentHeroIndex(
              (prev) =>
                (prev - 1 + DELHI_HERO_CAROUSEL.length) %
                DELHI_HERO_CAROUSEL.length
            )
          }
        ...
        <button
          ...
          onClick={() =>
            setCurrentHeroIndex(
              (prev) => (prev + 1) % DELHI_HERO_CAROUSEL.length
            )
          }
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2 px-3 py-1.5 rounded-full glass-chip border border-white/10">
          {DELHI_HERO_CAROUSEL.map((_, index) => (
            <button
              key={index}
              type="button"
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-6 border-b border-white/10">
          <div>
            ...
            <p className="text-sm text-text-mid mt-1">
              Filter historical monuments, thrill parks, shopping hubs, and iconic street food spots.
            </p>
          </div>

          {/* Category Tabs */}
          <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
            {DELHI_CATEGORIES.map((cat) => {
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                  ...
                  <div className="space-y-2 text-xs text-text-mid border-t border-white/10 pt-3">
                    <div className="flex items-center gap-2">
                      <Metro size={14} className="text-amber-400 shrink-0" />
                  ...
                  {/* Action Button */}
                  <Link
                    to={`/destination/north/delhi/${place.slug}`}
                    className={cn(
