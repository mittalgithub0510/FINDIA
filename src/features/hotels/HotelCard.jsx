...
export function HotelCard({ hotel, onSelect }) {
  ...
  return (
    <div className="bg-[#1B1613] border border-[#322A22] rounded-[14px] p-5 flex flex-col justify-between space-y-4 hover:border-[#C9A24B]/50 transition-colors duration-200 group shadow-md">
      <div className="space-y-3.5">
        {/* 1. Top Row: Tier Badge + Star Rating & Review Count */}
        ...
        <div className="p-3 rounded-xl bg-[#241E1A] border border-[#322A22] flex items-center justify-between gap-2">
          <div>
          ...
          {hotel.valueScore && (
            <div className="text-right">
              ...
              <span className="text-xs font-semibold text-[#5FA97C] bg-[#5FA97C]/10 border border-[#5FA97C]/30 px-2 py-0.5 rounded-md inline-block">
                {hotel.valueScore}/10
              </span>
            </div>
          )}
        ...
        {hotel.nearestAttractions && hotel.nearestAttractions.length > 0 && (
          <div className="pl-3 py-2 pr-2.5 rounded-lg bg-[#241E1A]/60 border border-[#322A22] border-l-2 border-l-[#C9A24B] flex items-center justify-between gap-2">
            ...
            <span className="text-[11px] font-medium text-[#5FA97C] bg-[#5FA97C]/15 border border-[#5FA97C]/30 px-2.5 py-0.5 rounded-full shrink-0">
        ...
        {hotel.amenities && (
          <div className="flex flex-wrap gap-1.5 pt-1">
            {hotel.amenities.slice(0, 4).map((amenity, idx) => (
              <span key={idx} className="text-[11px] px-2.5 py-1 rounded-full bg-[#241E1A] border border-[#322A22] text-[#9C9186]">
                {amenity}
              </span>
            ))}
            {hotel.amenities.length > 4 && (
              <span className="text-[11px] px-2.5 py-1 rounded-full bg-[#C9A24B]/15 border border-[#C9A24B]/40 text-[#C9A24B] font-medium">
      ...
      {/* 8. Single CTA Button: Gold bg, dark text, "View details" */}
      <button
        onClick={() => onSelect && onSelect(hotel)}
        className="w-full py-2.5 px-4 rounded-xl bg-[#C9A24B] hover:brightness-110 text-[#0F0D0B] text-xs font-semibold transition-all duration-150 text-center"
      >
        View details
      </button>
    </div>
