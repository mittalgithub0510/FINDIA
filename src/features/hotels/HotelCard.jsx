...
export function HotelCard({ hotel, onSelect }) {
  ...
  return (
    <div className="bg-[#1B1613] border border-[#2E271F] rounded-[16px] p-6 sm:p-7 flex flex-col justify-between space-y-5 hover:border-[#8A7238] transition-all duration-200 group shadow-lg">
      <div className="space-y-4">
        ...
        <h3 className="font-['Fraunces',serif] text-[25px] sm:text-[27px] font-medium text-[#F3EBDC] leading-snug tracking-tight group-hover:text-[#F3EBDC]/90 transition-colors">
          {hotel.name}
        </h3>
        ...
        <div className="p-3.5 rounded-xl bg-[#17130F] border border-[#2E271F] flex items-center justify-between gap-3">
          <div>
          ...
          {hotel.valueScore && (
            <div className="text-right">
              ...
              <span className="text-xs font-semibold text-[#5FA97C] bg-[#5FA97C]/10 border border-[#5FA97C]/30 px-2.5 py-0.5 rounded-full inline-block mt-0.5">
                {hotel.valueScore}/10
              </span>
            </div>
          )}
        ...
        {hotel.nearestAttractions && hotel.nearestAttractions.length > 0 && (
          <div className="pl-3.5 py-2.5 pr-3 rounded-lg bg-[#17130F] border border-[#2E271F] border-l-2 border-l-[#C9A24B] flex items-center justify-between gap-2">
        ...
        {hotel.amenities && (
          <div className="flex flex-wrap gap-1.5 pt-1">
            {hotel.amenities.slice(0, 4).map((amenity, idx) => (
              <span key={idx} className="text-[11px] px-3 py-1 rounded-full bg-[#17130F] border border-[#2E271F] text-[#9C9186]">
                {amenity}
              </span>
            ))}
            {hotel.amenities.length > 4 && (
              <span className="text-[11px] px-3 py-1 rounded-full bg-[#C9A24B]/[0.07] border border-[#8A7238] text-[#C9A24B] font-medium">
      ...
      <button
        ...
        className="w-full py-3 px-4 rounded-full bg-[#C9A24B] hover:brightness-110 text-[#1B1613] text-xs font-semibold transition-all duration-150 text-center cursor-pointer shadow-sm mt-2"
      >
        View details
      </button>
    </div>
